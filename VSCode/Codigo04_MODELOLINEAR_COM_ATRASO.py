...
Tambiente = 301.15  # 25 °C em Kelvin
Alpha = 0.01
Cp = 500
A = 0.0012
m = 0.004
...
try:
    for i in range(loops):
        ...
        # Modelo Não Linear
        if i <= 15:
            T1_ModeloNL[i] = T1[0]  # Condição inicial
        else:
    ...
    print(f'\nErro Médio Absoluto entre Temperatura Real e Modelo Não Linear Com Atraso: {erro_medio_NL:.2f} °C')
    print(f'Erro Médio Absoluto entre Temperatura Real e Modelo Linear Com Atraso: {erro_medio_L:.2f} °C')

    # Salvar gráfico em PNG
    plt.savefig('Grafico_TCLab_Modelo_Linear_Com_Atraso.png', dpi=300, bbox_inches='tight')

    # Salvar dados em .txt
    np.savetxt('Dados_simulacao_04.txt', np.column_stack((tm, T1, T1_ModeloNL, T1_ModeloL, Q1)),
