...
try:
    for i in range(loops):
        ...
        if i <= 0:
            ...
            T1_ModeloL[i] = T1_ModeloL[i] = T1_ModeloL[i-1] + 0.00499 * np.exp(-tm[i - 15] / 123) * Q_L[i] * np.heaviside(i - 15, 0)
    ...
    plt.savefig('Grafico_TCLab_Todos_Modelos_Comparados_Malha_Fechada_KP10.png', dpi=300, bbox_inches='tight')

    # Salvar dados em .txt
    np.savetxt('Dados_simulacao_13_Modelos_Comparados_Malha_Fechada_KP10.txt', np.column_stack((tm, T1, T1_ModeloNL, T1_ModeloL, T1_ModeloHAG, T1_ModeloSmith, T1_ModeloSundaresan, T1_ModeloZN, Q1)),
