...
try:
    for i in range(loops):
        ...
        plt.plot(tm[:i+1], T1_ModeloSmith[:i+1], 'g--', label='Modelo Smith', linewidth=2)
    ...
    plt.savefig('Grafico_TCLab_Modelo_Smith.png', dpi=300, bbox_inches='tight')

    # Salvar dados em .txt
    np.savetxt('Dados_simulacao_08_Smith.txt', np.column_stack((tm, T1, T1_ModeloSmith, Q1)),
               header='Tempo (s)\tTemperatura Real (°C)\tModelo Smith (°C)\tPotência (%)', fmt='%.2f', delimiter='\t')
