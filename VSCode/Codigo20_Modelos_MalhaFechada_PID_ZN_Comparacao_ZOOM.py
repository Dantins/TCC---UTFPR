...
try:
    for i in range(loops):
        ...
        # 🔍 Zoom na temperatura (não aplica ao gráfico de potência)
        inset_ax = inset_axes(ax1, width="30%", height="30%", loc="center", borderpad=2)
        inset_ax.grid()
        inset_ax.plot(tm[:i+1], T1[:i+1], 'r-', alpha=1)
        inset_ax.plot(tm[:i+1], T1_ModeloNL[:i+1], 'b--', alpha=0.8)
        inset_ax.plot(tm[:i+1], T1_ModeloL[:i+1], 'g-.', alpha=0.8)
        inset_ax.plot(tm[:i+1], T1_ModeloZN[:i+1], 'k-.', alpha=0.8)
        inset_ax.plot(tm[:i+1], T1_ModeloHAG[:i+1], 'c-.', alpha=0.8)
        inset_ax.plot(tm[:i+1], T1_ModeloSmith[:i+1], 'm-.', alpha=0.8)
        inset_ax.plot(tm[:i+1], T1_ModeloSundaresan[:i+1], 'y-.', alpha=0.8)
        inset_ax.set_xlim(0, 30)       # Limite do tempo para zoom
        inset_ax.set_ylim(24, 34)      # Limite de temperatura (não pega o setpoint)

        mark_inset(ax1, inset_ax, loc1=2, loc2=4, fc="none", ec="0.5")

        ax2 = plt.subplot(2, 1, 2)
        ax2.grid()
        ax2.plot(tm[:i+1], Q1[:i+1], 'r-')
        ax2.plot(tm[:i+1], Q_NL[:i+1], 'b--')
        ax2.plot(tm[:i+1], Q_L[:i+1], 'g-.')
        ax2.plot(tm[:i+1], Q_ZN[:i+1], 'k-.')
        ax2.plot(tm[:i+1], Q_HAG[:i+1], 'c-.')
        ax2.plot(tm[:i+1], Q_Smith[:i+1], 'm-.')
        ax2.plot(tm[:i+1], Q_Sundaresan[:i+1], 'y-.')
    ...
    plt.savefig('Grafico_TCLab_Todos_Modelos_Comparados_Malha_Fechada_PID_ZN.png', dpi=300, bbox_inches='tight')
    np.savetxt('Dados_simulacao_PID_ZN_ZOOM_COMPARADOS.txt',
